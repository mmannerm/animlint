...
Excerpts marked *real download* were captured once (July 2026) from
Mixamo's default Walking; unlike the stand-in transcripts they are not
regenerated by this repo's tests, so treat their exact numbers as a
snapshot.
...
(Counts vary by character. This download contains a second take named
`Take 001` with zero tracks and keys. Conversion omits that empty take,
writes the animated `mixamo.com` clip, and reports the omission. The
licensed download is not committed here.)
...
list will just be longer. From the real download (abridged):

```console
$ animsmith inspect walking.glb                   # real download
walking.glb
rig profile: mixamo (9 roles)
  ...the same nine-role table as above...
skeleton: 70 bones
  <fbx-root>
...
The extra bones — fingers, twist joints, end markers — carry no role,
and the two mesh nodes ride along for `report` playback.
...
```console
$ animsmith lint walking.glb                      # real download
walking.glb:
...
download above is Mixamo's *default* Walking — the traveling one — and
under this In Place contract it fails exactly as it should:

```console
$ animsmith lint walking.glb                      # real download
walking.glb:
  error[in-place] clip 'mixamo.com': declared in-place but the root travels
    at 1.79 m/s — the character will glide at runtime (measured 1.7857,
    expected 0.0000)
  ...
```
...
$ animsmith lint walking.glb                      # real download
walking.glb:
  warning[foot-slide] clip 'mixamo.com' bone 'mixamorig:LeftFoot' @0.333s:
